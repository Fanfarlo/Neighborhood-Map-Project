function appViewModel() {
    ...
    this.goToMarker = function(clickedDeal) {
        ...
        for (var key in self.markers()) {
            if (clickedDealName === self.markers()[key].marker.title) {
                map.panTo(self.markers()[key].marker.position);
                map.setZoom(14);
                infowindow.setContent(self.markers()[key].content);
    ...
    this.processLocationSearch = function() {
        //jQuery selector instead of KO binding because this field is affected by the autocomplete plugin.
        self.searchStatus('');
    ...
    this.filterResults = function() {
        ...
        if (!searchWord) {
            ...
            //with any venue name or dealTags in the list, if so push that object to the list
                ...
                if (array[i].dealName.toLowerCase().indexOf(searchWord) != -1) {
                    ...
                    for (var j = 0; j < array[i].dealTags.length; j++) {
                        if (array[i].dealTags[j].name.toLowerCase().indexOf(searchWord) != -1) {
                            self.markers()[i].marker.setMap(map);
                            self.list.push(array[i]);
                            //otherwise hide list all other markers from the map
                        } else {
